...
void xAna_mc(const char** inpaths, int npaths) {
  ...
  TFile* file = TFile::Open("output_mc.root", "RECREATE");
  ...
  //for (Long64_t ev = 0; ev < 600000; ++ev) {
    ...
    // nmc = genElectrons.size();
    // mc_pt.clear();
    // mc_eta.clear();
    // mc_phi.clear();
    // for(int i = 0; i != (int)genElectrons.size(); ++i) {
      // mc_pt.push_back(genElectrons[i].Pt());
      // mc_eta.push_back(genElectrons[i].Eta());
      // mc_phi.push_back(genElectrons[i].Phi());
    // }
    ...
    UShort_t* eleID = (UShort_t*)data.GetPtrShort("eleIDbit");
    ...
    bool twoElectrons  = nele > 1 && ele_pt[1] > 10.0;// && ele_id[0] == true && ele_id[1] == true;
    bool electronAndHF = nele > 0 && ele_pt[0] > 10.0 && nhf > 0;// && ele_id[0] == true && nhf > 0;
    bool twoHF         = nhf > 1;
