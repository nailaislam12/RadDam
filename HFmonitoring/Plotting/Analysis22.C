#define Analysis22_cxx
#include "Analysis22.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void Analysis22::Loop() {
  ...
  // Setup for deriving Raddam Factors
  TH1F* temp;
  ...
  for (int i = 0; i < numfactors; ++i) { // Loop over factors
    ...
    for (int j = 30; j <= 42; ++j) { // Loop over ietas (plus and minus)
      ...
      temp = new TH1F( ("h_mass_etaMinus" + to_string(j) + "_Xn" + to_string(i)).c_str(), "", 140, 20, 160);
      h_masses[-j].push_back( temp);
    }
  }
  if (numfactors == 0) 
  ...
  TH1F* h_ele_pt  = new TH1F("h_ele_pt", "ECAL Ele p_{T}", 100, 0, 100);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    // Need to declare these outside now...
    double longFiberEn;
    double shortFiberEn;
    double Mass;
    TLorentzVector e1pu;
	...
	continue;
      ...
      iEta34Plus = eta > 3.664 && eta < 3.839;
      iEta35Plus = eta > 3.839 && eta < 4.013;
      iEta36Plus = eta > 4.013 && eta < 4.191;
      iEta37Plus = eta > 4.191 && eta < 4.363;
      iEta38Plus = eta > 4.363 && eta < 4.538;
      ...
      iEta34Minus = eta < -3.664 && eta > -3.839;
      iEta35Minus = eta < -3.839 && eta > -4.013;
      iEta36Minus = eta < -4.013 && eta > -4.191;
      iEta37Minus = eta < -4.191 && eta > -4.363;
      iEta38Minus = eta < -4.363 && eta > -4.538;
