...
void MakePNGPlots22() {
  ...
  // Make Changes Here
  TString figdir = "Figures25C_noPU_CB_Gauss/";
  TString year = "2025C";
  TFile *fMC = new TFile("outplots/CB_Gauss_25C/output_mc_PLOTS_mc_noPU.root");
  TFile *fData = new TFile("outplots/CB_Gauss_25C/output_data_2025C_PLOTS_data_noPU.root");
  ...
  TF1* funcCB = new TF1("fitCB", "crystalball", 20, 160); 
  funcCB->SetLineWidth(2);
  funcCB->SetParameters(270, 80, 4, -1.5, 2.0);
  canvas->SetLogy(0);
  ...
  for(int i = 30; i <= 41; ++i) {
    ...
    hEtaPlusMC->Draw();
    if ( i == 43){
    funcGaus->SetRange(hEtaPlusMC->GetMaximumBin()+5,hEtaPlusMC->GetMaximumBin()+70);
    }else{
    ...
    hEtaPlusMC->Draw();
    //funcGaus->SetRange(hEtaPlusMC->GetMaximumBin()+5,hEtaPlusMC->GetMaximumBin()+35);
    funcCB->SetRange(hEtaPlusMC->GetMaximumBin()+5,hEtaPlusMC->GetMaximumBin()+50);
    funcCB->SetParNames("Norm", "Mean", "Sigma", "Alpha", "n");
    funcCB->SetParLimits(0, 50, 100000);
    funcCB->SetParLimits(1, 75, 95);
    funcCB->SetParLimits(2, 6, 10.0);
    funcCB->SetParLimits(3, -10, -0.5);
    funcCB->SetParLimits(4, 0.5, 3.0);
    TFitResultPtr FitResultEtaPlusMC = hEtaPlusMC->Fit("fitCB","RSQ");
    FitResultEtaPlusMC->Print();
    double mcPlus       = funcCB->GetParameter(1);
    double mcPlusErr    = funcCB->GetParError(1);
    double mcWidthPlus  = funcCB->GetParameter(2);
    double mcWidthPlusErr = funcCB->GetParError(2);
    TString EntriesEtaPlusMC = TString::Format("Entries: %.0f", hEtaPlusMC->GetEntries());
    TString MeanEtaPlusMC    = TString::Format("#mu = %.3f #pm %.3f", mcPlus, mcPlusErr);
    TString SigmaEtaPlusMC   = TString::Format("#sigma = %.3f #pm %.3f", mcWidthPlus, mcWidthPlusErr);
    TString Chi2EtaPlusMC    = TString::Format("#chi^{2} = %.2f", FitResultEtaPlusMC->Chi2());
    TString NDOFEtaPlusMC    = TString::Format("ndf = %.0u", FitResultEtaPlusMC->Ndf());
    TLatex TextEtaPlusMC = TLatex(); 
    TextEtaPlusMC.SetTextFont(42);     
    ...
    gTemplateFit = (TF1*)hEtaPlusMCTemplate->GetFunction("fitCB"); 		
    //Signal
    ...
    TH1F *hEtaMinusMC = (TH1F*)fMC->Get(EtaMinusNum);
    hEtaMinusMC->Draw();
    funcCB->SetRange(hEtaMinusMC->GetMaximumBin()+5,hEtaMinusMC->GetMaximumBin()+50);
    funcCB->SetParNames("Norm", "Mean", "Sigma", "Alpha", "n");
    funcCB->SetParLimits(0, 50, 100000);
    funcCB->SetParLimits(1, 75, 95);
    funcCB->SetParLimits(2, 6, 10.0);
    funcCB->SetParLimits(3, -10, -0.5);
    funcCB->SetParLimits(4, 0.5, 3.0);
    TFitResultPtr FitResultEtaMinusMC = hEtaMinusMC->Fit("fitCB","RSQ");
    FitResultEtaMinusMC->Print();
    double mcMinus       = funcCB->GetParameter(1);
    double mcMinusErr    = funcCB->GetParError(1);
    double mcWidthMinus  = funcCB->GetParameter(2);
    double mcWidthMinusErr = funcCB->GetParError(2);
    TString EntriesEtaMinusMC = TString::Format("Entries: %.0f", hEtaMinusMC->GetEntries());
    TString MeanEtaMinusMC    = TString::Format("#mu = %.3f #pm %.3f", mcMinus, mcMinusErr);
    TString SigmaEtaMinusMC   = TString::Format("#sigma = %.3f #pm %.3f", mcWidthMinus, mcWidthMinusErr);
    TString Chi2EtaMinusMC    = TString::Format("#chi^{2} = %.2f", FitResultEtaMinusMC->Chi2());
    TString NDOFEtaMinusMC    = TString::Format("ndf = %.0u", FitResultEtaMinusMC->Ndf());
    TLatex TextEtaMinusMC = TLatex(); 
    TextEtaMinusMC.SetTextFont(42);     
    ...
    hEtaMinusData->Draw();
    if (i == 43){
    funcGaus->SetRange(hEtaMinusData->GetMaximumBin()+5,hEtaMinusData->GetMaximumBin()+70);
    }else{
    ...
    gTemplateFitM = (TF1*)hEtaMinusMCTemplate->GetFunction("fitCB");
    ...
    combinedFuncM->SetParLimits(1, -5.0, 5.0);
