...
void MakePNGPlots22() {
  ...
  // Make Changes Here
  TString figdir = "Figures25C_noPU_overlay/";
  TString year = "2025C";
  TFile *fMC = new TFile("outplots/HF_25C/output_mc_PLOTS_mc_noPU.root");
  TFile *fData = new TFile("outplots/HF_25C/output_data_2025C_PLOTS_data_noPU_RaddCorr.root");
  ...
  for(int i = 30; i <= 41; ++i) {
    ...
    // === Overlay Data vs MC for EtaPlus ===
    hEtaPlusMC->SetLineColor(kBlue);
    hEtaPlusMC->SetLineWidth(1);
    hEtaPlusMC->SetLineStyle(1);
    //hEtaPlusMC->SetMarkerStyle(24);
    //hEtaPlusMC->SetMarkerColor(kBlue);
    hEtaPlusData->SetLineColor(kRed);
    hEtaPlusData->SetLineWidth(1);
    hEtaPlusData->SetLineStyle(1);
    //hEtaPlusData->SetMarkerStyle(20);
    ...
    hEtaPlusData->Draw("E SAME");
    TLegend* legOverlay = new TLegend(0.6, 0.75, 0.85, 0.88);
    legOverlay->AddEntry(hEtaPlusData, year + " Data", "lep");
    legOverlay->AddEntry(hEtaPlusMC, "MC", "lep");
    ...
    latexOverlay.DrawLatex(0.6, 0.70, TString::Format("Data/MC scale = %.3f", scaleFactor));
    CMSlabel.DrawLatex(0.128,0.955,"#bf{CMS} #it{Preliminary}");
    ...
    //hEtaMinusMC->SetMarkerStyle(24);
    //hEtaMinusMC->SetMarkerColor(kBlue);
    hEtaMinusData->SetLineColor(kRed);
    hEtaMinusData->SetLineWidth(1);
    ...
    hEtaMinusData->Draw("E SAME");
    TLegend* legOverlayMinus = new TLegend(0.6, 0.75, 0.85, 0.88);
    legOverlayMinus->AddEntry(hEtaMinusData, year + " Data", "lep");
    legOverlayMinus->AddEntry(hEtaMinusMC, "MC", "lep");
    ...
    latexOverlayMinus.DrawLatex(0.6, 0.70, TString::Format("Data/MC scale = %.3f", scaleFactorMinus));
    CMSlabel.DrawLatex(0.128,0.955,"#bf{CMS} #it{Preliminary}");
